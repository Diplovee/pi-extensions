...
The extension provides `browser_*` tools for navigation, screenshots, page content, clicks, reliable typing, keyboard input, scrolling, history, zoom, recovery, device emulation, arbitrary viewport resizing, and console inspection/evaluation. `browser_device` supports `desktop`, `desktop-hidpi`, `iphone-13`, `iphone-15`, `iphone-15-landscape`, `pixel-7`, `pixel-7-landscape`, `ipad`, and `ipad-landscape`. Device switching applies the preset viewport, user-agent, mobile behavior, touch support, and device pixel ratio, then reopens the current URL while retaining the persistent profile. `browser_page_info` reports the active device, profile, viewport, user-agent, touch support, and device pixel ratio.

When a prompt mentions Burawuza, browser testing, responsive behavior, or a local web app, the extension adds workflow guidance: read `browser_memory` first, inspect project docs for the app-server command, start and verify the server with Bash when needed, select the requested device before navigation, and verify the resulting device/viewport. The browser itself starts automatically on the first browser tool call.

Browser tool results automatically maintain a bounded project-local log at `.pi/burawuza/`: `knowledge.json` stores the latest compact page summaries and hashes, `events.jsonl` stores the latest 100 actions/change records, and `screenshots/` keeps at most eight recent screenshots within a size limit when `browser_screenshot({ remember: true })` is explicitly requested. `browser_content({ remember: true })` can save a short redacted preview of stable labels/content. Screenshots can contain visible secrets, so they are not saved by default. Typed values are recorded only as lengths, URLs are stored without query strings, and summaries/console text are redacted and truncated. `browser_memory({ action: "read" })` returns only a compact readback; treat it as a hint and verify the live UI because the log records changes when page content differs.
