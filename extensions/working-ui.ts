...
type WorkingSelection = { kind: "mode"; mode: WorkingMode } | { kind: "reset" } | { kind: "custom" } | null;
...
export default function workingUI(pi: ExtensionAPI) {
	...
	const showWorkingDialog = async (ctx: ExtensionContext): Promise<WorkingSelection> => {
		...
		return await ctx.ui.custom<WorkingSelection>(
			(tui, theme, _kb, done) => {
				let index = 0;
	...
	pi.registerCommand("working", {
		description:
			"Customize the loading text. Usage: /working [show|set <text>|mode <funny|clean|serious|chaos>|reset]",
